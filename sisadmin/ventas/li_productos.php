	...
	<tr>
			...
			<td align="center" >
              	<select name="<?php echo $id."|".$talla; ?>-tallas" id="<?php echo $id."|".$talla; ?>-tallas" disabled>
					...
					{
					?>
					<option <?php if($talla == $result_tallas_row['idtallas']){ echo "selected"; } ?> value="<?php echo $result_tallas_row['idtallas']; ?>"><?php echo utf8_encode($result_tallas_row['valor']." ".$result_tallas_row['unidad']); ?></option>
					<?php
					}while($result_tallas_row = $db->fetch_assoc($result_tallas));
					?>
            	</select>
            </td>
            <td align="center" ><input type="button" name="button" id="button" value="Agregar" onClick="S_Producto('<?php echo $id;?>',document.getElementById('<?php echo $id."|".$talla; ?>'+'-cantidad').value,'<?php echo $idsucursales; ?>',document.getElementById('<?php echo $id."|".$talla; ?>'+'-tallas').value); CerrarModalGeneral('ModalPrincipal');" <?php echo $desactivado; ?> ></td>
      </tr>
