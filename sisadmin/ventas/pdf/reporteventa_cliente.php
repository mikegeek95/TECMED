...
$sql_venta = $f->desconver_especial($_GET['sql_venta']);
$sql_devo = $f->desconver_especial($_GET['sql_devo']);

//die($sql_venta." y  ".$sql_devo);
...

{
	...
	$result_sucursales_num = $this->db->num_rows($result_sucursales);
	...
	$this->SetX(50);	
	$this->Cell(100,5,"Sucursal ".$result_mi_sucursal_row['sucursal'],0,1);	
	$this->SetX(50);
	...
	$this->SetFont('Arial','',8);
	$this->SetX($x);	
	$this->Cell(100,5,"Sucursal ".$result_sucursales_row['sucursal'],0,1);	
	$this->SetX($x);
...
/*$pdf->SetX(90);
$pdf->Cell(70,7,"REPORTE DE CLIENTE",0,0);
$pdf->Ln();


$pdf->SetX(20);
$pdf->Cell(100,5,utf8_decode("FECHA INICIO : ").$ventas->id_notaremision);
$pdf->Ln();

$pdf->SetX(20);
$pdf->Cell(100,5,utf8_decode("FECHA FIN : ").$ventas->id_notaremision);
$pdf->Ln();
*/


$pdf->SetX(100);
$pdf->Cell(150,7,"VENTAS",0,0);
$pdf->Ln();
...
$pdf->Ln();
	$pdf->SetFontSize(9);
	$pdf->SetX(20);
	$pdf->Cell(20,5,"ORD COMP",1,0,'C');
	$pdf->Cell(35,5,"FECHA",1,0,'C');
	$pdf->Cell(69,5,"NOMBRE",1,0,'C');
	$pdf->Cell(26,5,"ESTATUS",1,0,'C');
...
do
{
	...
	$pdf->SetX(20);
	$pdf->SetFontSize(9);
	$pdf->Cell(20,5,$result_venta_row['idnota_remision'],1,0,'C');
	$pdf->Cell(35,5," ".strtoupper($result_venta_row['fechapedido']),1,0,'C');
	$pdf->Cell(69,5,($result_venta_row['nombre']." ".$result_venta_row['paterno']." ".$result_venta_row['materno']),1,0,'C');
	$pdf->Cell(26,5,$estatus[$result_venta_row['estatus']],1,0,'C');
	$pdf->Cell(26,5,"$ ".number_format($result_venta_row['total'],2,'.',','),1,0,'C');
	$pdf->Ln();
...
$pdf->SetX(20);
		...
		$pdf->Cell(35,5,"",0,0,'C');
		$pdf->Cell(69,5,"",0,0,'C');
		$pdf->Cell(26,5,"TOTAL",0,0,'R');
		$pdf->Cell(26,5,"$ ".number_format($total,2,'.',','),0,0,'C');
...
$pdf->SetX(94);
$pdf->Cell(100,7,"DEVOLUCIONES",0,0);
$pdf->Ln();
...
do
  {
		...
		$pdf->SetX(20);
		$pdf->SetFontSize(9);
		$pdf->Cell(20,5,$result_devo_row['idcliente_devolucion'],1,0,'C');
		$pdf->Cell(20,5,$result_devo_row['idnota_remision'],1,0,'C');
		$pdf->Cell(35,5,strtoupper($result_devo_row['fecha']),1,0,'C');
		$pdf->Cell(80,5,strtoupper($result_devo_row['nombre']." ".$result_devo_row['paterno']." ".$result_devo_row['materno']),1,0,'C');
		$pdf->Cell(20,5,"$ ".number_format($result_devo_row['total'],2,'.',','),1,0,'C');
		$pdf->Ln();
	 ...
	 $pdf->SetX(20);
		...
		$pdf->Cell(20,5,"$ ".number_format($total2,2,'.',','),0,0,'C');
...
$pdf->SetX(90);
$pdf->Cell(100,7,"DESGLOSE DE TOTALES",0,0);
$pdf->Ln();
...
$pdf->Ln();
	$pdf->SetFontSize(9);
	$pdf->SetX(20);
	$pdf->Cell(58,5,"VENTAS",1,0,'C');
	$pdf->Cell(58,5,utf8_decode("DEVOLUCIÓN"),1,0,'C');
	$pdf->Cell(58,5,"TOTAL",1,0,'C');
$pdf->Ln();

					
	$pdf->SetX(20);
	$pdf->SetFontSize(9);
	$pdf->Cell(58,5,"$ ".number_format($total,2,'.',','),1,0,'C');
	$pdf->Cell(58,5,"$ ".number_format($total2,2,'.',','),1,0,'C');
	$pdf->Cell(58,5,"$ ".number_format(($total - $total2),2,'.',','),1,0,'C');
	$pdf->Ln(5);
